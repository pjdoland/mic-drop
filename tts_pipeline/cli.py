...
def _load_config() -> dict:
    """Read key=value pairs from ``.mic-drop.env`` in the project root.

    The file is generated by ``setup.sh`` when the user picks a cache
    directory.  Lines starting with ``#`` and blank lines are skipped.
    """
    env_path = Path(__file__).resolve().parent.parent / ".mic-drop.env"
    config: dict = {}
...
def _apply_config_defaults(args: argparse.Namespace) -> None:
    """Back-fill CLI args from ``.mic-drop.env`` when the user didn't pass them."""
    if args.cache_dir is not None:
        return                          # explicit flag wins
    raw = _load_config().get("TORTOISE_CACHE_DIR")
    if raw:
        args.cache_dir = Path(raw)
        logger.debug("cache_dir from .mic-drop.env: %s", args.cache_dir)
...
def main() -> None:
    ...
    # Fill in defaults from .mic-drop.env (setup.sh writes this file)
